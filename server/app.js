...
app.get("/", async (req, res) => {
  try {
    ...
    const [kpiAvui, kpiMes, stockBaix, ultimesVendes, topProductes] =
      await Promise.all([
        db.query(
          `SELECT COUNT(*) as total FROM sales WHERE DATE(sale_date)=${avui}`,
        ),
        db.query(
          `SELECT COUNT(*) as total FROM sales WHERE sale_date LIKE '${mesAny}'`,
        ),
        db.query(`SELECT name, stock FROM products WHERE stock <= 5`),
        db.query(
          `SELECT s.id, DATE(s.sale_date) as data, c.name as client, s.total FROM sales s JOIN customers c ON s.customer_id=c.id ORDER BY s.sale_date DESC LIMIT 5`,
        ),
        db.query(
          `SELECT p.name, SUM(si.qty) as total_vendut FROM sale_items si JOIN products p ON si.product_id=p.id GROUP BY p.id ORDER BY total_vendut DESC LIMIT 5`,
        ),
      ]);
    ...
    const data = {
      ...
      ultimesVendes: db.table_to_json(ultimesVendes, {
        id: "number",
        data: "string",
        client: "string",
        total: "number",
      }),
      topProductes: db.table_to_json(topProductes, {
        name: "string",
        total_vendut: "number",
      }),
...
app.get("/productes", async (req, res) => {
  try {
    ...
    const [products, count] = await Promise.all([
      db.query(
        `SELECT * FROM products ${where} ORDER BY id LIMIT ${limit} OFFSET ${offset}`,
      ),
...
app.get("/clients", async (req, res) => {
  try {
    ...
    if (cerca)
      where += `WHERE name LIKE '%${cerca}%' OR email LIKE '%${cerca}%'`;
    if (vip)
      where +=
        (where ? " AND " : "WHERE ") +
        "(SELECT SUM(total) FROM sales WHERE customer_id=c.id) > 100";

    const [clients, count] = await Promise.all([
      db.query(
        `SELECT c.*, COALESCE(SUM(s.total), 0) as total_gastat, COUNT(s.id) as num_compres FROM customers c LEFT JOIN sales s ON c.id=s.customer_id ${where} GROUP BY c.id ORDER BY c.id LIMIT ${limit} OFFSET ${offset}`,
      ),
      db.query(
        `SELECT COUNT(DISTINCT c.id) as total FROM customers c ${where}`,
      ),
    ]);

    res.render("clients/list", {
      clients: db.table_to_json(clients, {
        id: "number",
        name: "string",
        email: "string",
        phone: "string",
...
// Proveidors
app.get("/proveidors", async (req, res) => {
  try {
    const page = parseInt(req.query.pagina || 0);
    const limit = 10;
    const offset = page * limit;
    const cerca = req.query.cerca || "";
    const where = cerca
      ? `WHERE s.name LIKE '%${cerca}%' OR s.email LIKE '%${cerca}%'`
      : "";

    const [proveidors, count] = await Promise.all([
      db.query(
        `SELECT s.id, s.name, s.email, s.phone, COUNT(p.id) as num_productes
         FROM suppliers s
      ...
      db.query(`SELECT COUNT(*) as total FROM suppliers s ${where}`),
    ]);

    res.render("proveidors/list", {
      proveidors: db.table_to_json(proveidors, {
        id: "number",
        name: "string",
        email: "string",
        phone: "string",
        num_productes: "number",
      }),
      pagina: page,
      totalPages: Math.ceil(count[0].total / limit),
      cerca,
    ...
    console.error(err);
    res.status(500).send("Error proveidors");
  }
});

app.get("/proveidorAfegir", async (req, res) => {
  res.render("proveidors/form", { proveidor: {}, common: getCommonData() });
});

app.get("/proveidorEditar", async (req, res) => {
  try {
    const id = parseInt(req.query.id, 10);
    if (!Number.isInteger(id) || id <= 0)
      return res.status(400).send("ID invàlid");

    const proveidors = await db.query(
      `SELECT * FROM suppliers WHERE id = ${id} LIMIT 1`,
    );
    if (!proveidors.length) return res.status(404).send("Proveïdor no trobat");

    res.render("proveidors/form", {
      proveidor: db.table_to_json(proveidors, {
        id: "number",
        name: "string",
        email: "string",
        phone: "string",
...
app.get("/clientEditar", async (req, res) => {
  try {
    ...
    res.render("clients/form", {
      client: db.table_to_json(clients, {
        id: "number",
        name: "string",
        email: "string",
        phone: "string",
...
app.get("/clientFitxa", async (req, res) => {
  try {
    ...
    const [client, vendes] = await Promise.all([
      ...
      db.query(
        `SELECT s.id, DATE_FORMAT(s.sale_date, '%d/%m/%Y') as sale_date, s.total FROM sales s WHERE s.customer_id = ${id} ORDER BY s.sale_date DESC LIMIT 10`,
      ),
    ]);

    if (!client.length) return res.status(404).send("Client no trobat");

    const totalGastat = await db.query(
      `SELECT COALESCE(SUM(total), 0) as total FROM sales WHERE customer_id = ${id}`,
    );
    const ticketMitja =
      totalGastat[0].total /
        (
          await db.query(
            `SELECT COUNT(*) as total FROM sales WHERE customer_id = ${id}`,
          )
        )[0].total || 0;

    res.render("clients/fitxa", {
      client: db.table_to_json(client, {
        id: "number",
        name: "string",
        email: "string",
        phone: "string",
      })[0],
      vendes: db.table_to_json(vendes, {
        id: "number",
        sale_date: "string",
        total: "number",
      }),
...
app.get("/vendes", async (req, res) => {
  try {
    ...
    const [vendes, count] = await Promise.all([
      ...
      db.query(
        `SELECT COUNT(*) as total FROM sales s JOIN customers c ON s.customer_id = c.id ${where}`,
      ),
    ]);
...
app.get("/vendaAfegir", async (req, res) => {
  try {
    ...
    const [clients, productes] = await Promise.all([
      ...
      db.query(
        `SELECT id, name, price, stock FROM products WHERE active = 1 AND stock > 0 ORDER BY name`,
      ),
    ]);

    res.render("vendes/form", {
      clients: db.table_to_json(clients, { id: "number", name: "string" }),
      productes: db.table_to_json(productes, {
        id: "number",
        name: "string",
        price: "number",
        stock: "number",
...
app.get("/vendaFitxa", async (req, res) => {
  try {
    ...
    res.render("vendes/fitxa", {
      ...
      linies: db.table_to_json(linies, {
        product_name: "string",
        qty: "number",
        unit_price: "number",
        line_total: "number",
...
app.post("/createVenda", async (req, res) => {
  try {
    ...
    const insertSale = await db.query(
      `INSERT INTO sales (customer_id, sale_date, payment_method, total) VALUES (${customer_id}, NOW(), "${payment_method}", ${totalVenda})`,
    );
    ...
    for (let i = 0; i < prodIds.length; i++) {
      ...
      await db.query(
        `INSERT INTO sale_items (sale_id, product_id, qty, unit_price, line_total) VALUES (${sale_id}, ${pId}, ${q}, ${preuUnitari}, ${q * preuUnitari})`,
      );
